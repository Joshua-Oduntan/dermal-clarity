...
class Predictor:
    def __init__(self) -> None:
        ...
        self.ensemble_models = ["MobileNet", "VGG19", "InceptionV3"]
    ...
    def predict(self, image_bytes: bytes, model_name: str) -> dict[str, Any]:
        ...
        model_name = model_name or "MobileNet"
        if model_name == "Ensemble":
            probabilities = self._ensemble_predict(image_bytes, trace_id)
        else:
    ...
    def _ensemble_predict(self, image_bytes: bytes, trace_id: str) -> np.ndarray:
        logger.info("[%s] performing ensemble prediction", trace_id)
        predictions = []
        for model_name in self.ensemble_models:
            tensor = self.preprocessor.preprocess(image_bytes, model_name)
            logger.info("[%s] preprocessing complete for ensemble member %s", trace_id, model_name)
            model = self.model_loader.get_model(model_name)
            logger.info("[%s] loading ensemble model: %s", trace_id, model_name)
            probabilities = np.asarray(model.predict(tensor, verbose=0)).squeeze(axis=0).astype(np.float32)
            if probabilities.ndim != 1:
                probabilities = probabilities.reshape(-1)
            if probabilities.size > len(CLASS_NAMES):
            ...
            if probabilities.size != len(CLASS_NAMES):
                raise ValueError(
                    f"Ensemble model output size mismatch for {model_name}: expected {len(CLASS_NAMES)} classes, got {probabilities.size}"
                )
            predictions.append(probabilities)

        if not predictions:
            raise RuntimeError("No ensemble models are available for prediction")

        ensemble_scores = np.mean(np.vstack(predictions), axis=0).astype(np.float32)
        logger.info("[%s] ensemble probabilities computed", trace_id)
